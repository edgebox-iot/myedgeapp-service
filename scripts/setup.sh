...
echo " -> Updating packages / enabling universe repo"
...
if [ -z "$BOOTNODE_IP" ]; then
    echo "\$BOOTNODE_IP is empty. Skipping tinc-boot generation and service start"
else
...
mv /tmp/etcd-download/etc* /usr/local/bin
